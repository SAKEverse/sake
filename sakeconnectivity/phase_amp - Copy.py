...
def tort_mi(phase_array, amp_array):
    ...
    phasebins = np.arange(-np.pi,np.pi,0.1)                     # define phase bins
...
class PhaseAmp:
    ...
    def get_event(self, df):
        ...
        out.insert(0,'window', value=[bins[idx]/int(self.fs) for idx in range(len(bins))])
        out.insert(0,'bandcomp', value=df.bandcomp.unique()[0])
        out.insert(1,'phasereg_ampreg', value=df.regioncomp.unique()[0])
...
def phaseamp_batch(downsampled_df, iter_freqs, fs, windowsize):
    ...
    for (animal, file_name, start_time), data in tqdm(animal_df, desc='Calculating Phase Amp:'):
        ...
        pa_couples.reset_index(inplace=True)
        pa_couples[group_cols] = data[group_cols].iloc[0]
        all_pa_couples = pd.concat((all_pa_couples, pa_couples))
        
    return all_pa_couples
